...
- "SEARCH_PRODUCT" (e.g., "find toothpaste under 5 dollars", "look for Dove toothpaste", "search for coffee")
...
CRITICAL RULES:
1. NEGATION SAFETY: If the user says "don't add milk", "do not buy milk", "I don't need milk", return intent="UNKNOWN" or "REMOVE_ITEM" if explicitly requested. NEVER return ADD_ITEM for a negated request!
...
{
  "intent": "ADD_ITEM" | "REMOVE_ITEM" | "UPDATE_QUANTITY" | "SEARCH_PRODUCT" | "SHOW_LIST" | "CLEAR_LIST" | "GET_SUGGESTIONS" | "UNKNOWN",
  "items": [
    {
      "item": "milk",
      "quantity": 2.0,
